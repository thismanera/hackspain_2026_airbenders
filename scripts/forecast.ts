...
async function forecastParams(): Promise<ForecastParameters> {
  return forecastParametersSchema.parse(
    JSON.parse(await readFile(forecastParameterPath(), "utf8")),
  );
}
      ...
      p.history.map((f) =>
        f
          ? {
              ...
              cobrosOp: f.cobrosOp,
              pagosOp: f.pagosOp,
              servicioDeuda: f.servicioDeuda,
              dispCredito: f.dispCredito,
              amortCredito: f.amortCredito,
            }
          : undefined,
      ),
    ]),
...
async function doFit() {
  ...
  const summary = {
    ...
    conectadoSolo: fp.solo.conectado,
    conectadoGrupo: fp.grupo.conectado,
    ajuste: fp.ajuste,
    ajusteSolo: fp.solo.ajuste,
    ajusteGrupo: fp.grupo.ajuste,
...
async function doRun() {
  ...
  const summary = {
    ...
    groups: byGroup.size,
    version: fp.version,
    metodoSolo: fp.solo.conectado ? "v1_proyeccion" : "desconectado",
    metodoGrupo: fp.grupo.conectado ? "v1_proyeccion" : "desconectado",
  };
  console.log(JSON.stringify(summary));
}
...
async function doBacktest() {
  ...
  const report = {
    ...
    scoreSolo: backtestForecast(scores, forecasts, con, sin, VENTANAS, "scoreSolo"),
    scoreGrupo: backtestForecast(scores, forecasts, con, sin, VENTANAS, "scoreGrupo"),
  };
