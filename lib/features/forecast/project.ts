...
function mediaFlujoProyectada(
  ctx: CompanyCtx,
  h: number,
  key: "cobrosOp" | "pagosOp" | "servicioDeuda",
) {
  const current = ctx.flows[ctx.t];
  if (!current?.observed) return null;
  const serieActual = tendencia(
    serie(ctx, (m) => (ctx.flows[m]?.observed ? (ctx.flows[m]![key] ?? 0) : null)),
  );
  const values: number[] = [];
  for (let m = Math.max(0, ctx.t - 5); m <= ctx.t + h; m++) {
    if (m <= ctx.t) {
      const f = ctx.flows[m];
      if (f?.observed) values.push(f[key] ?? 0);
    } else {
      const offset = m - ctx.t;
      values.push((current[key] ?? 0) + serieActual.tendencia * acumulada(offset));
    }
  }
...
export function proyectarEmpresa(ctx: CompanyCtx, h: number, clip: Clip): Proyeccion {
  ...
  return {
    ...
    capacidadNetaPred:
      (mediaFlujoProyectada(ctx, h, "cobrosOp") ?? 0) -
      (mediaFlujoProyectada(ctx, h, "pagosOp") ?? 0) -
      (mediaFlujoProyectada(ctx, h, "servicioDeuda") ?? 0),
    sinTendencia,
  };
}
