...
test("grupo_aval: rich sibling lifts a weak subsidiary, capped by w", () => {
  const filial = member({
    ...
    cobrosOp12m: 240_000,
    pagosOp12m: 252_000,
    obligacionesMedia6m: 1_000,
    intragrupoIn12m: 60_000,
  });
  const hermana = member({
    company: "h",
    scoreSolo: 80,
    cobrosOp12m: 6_000_000,
    pagosOp12m: 4_200_000,
    capacidadCuotaAdv: 50_000,
    intragrupoOut12m: 60_000,
  });
...
test("grupo_contagio: weak sibling drags a healthy one without capacity filter", () => {
  const filial = member({
    company: "f",
    scoreSolo: 78,
    cobrosOp12m: 1_200_000,
    pagosOp12m: 1_020_000,
    intragrupoOut12m: 180_000,
  });
  const hermana = member({
    company: "h",
    scoreSolo: 30,
