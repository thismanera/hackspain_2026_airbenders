...
let peerSpaceCache: PeerSpace | null = null;
...
function hasData(month: MonthScore): boolean {
  return month.coverage.observedMonths > 0;
}
 ...
 */
function peerSpace(): PeerSpace {
  if (peerSpaceCache) return peerSpaceCache;
  const portfolio = buildPortfolio();
  const ids = [...portfolio.keys()];

  const rows: (number | null)[][] = [];
  for (const dataset of portfolio.values()) {
    for (const month of dataset.months) if (hasData(month)) rows.push(subscoresOf(month));
  }
  const fit = pcaFit(rows, 3);

  const coords = new Map<string, (Vec3 | null)[]>();
  for (const id of ids) {
    const dataset = portfolio.get(id)!;
    coords.set(
      id,
  ...
  CALENDAR.forEach((_, index) => {
    ...
    const clusters: PeerCluster[] = result.centroids.map((centroid, c) => {
      ...
      const memberZ = members.map((id) =>
        standardizeRow(subscoresOf(portfolio.get(id)!.months[index]), fit.mean, fit.std),
      );
      return {
        id: c,
        label: clusterLabel(dominantTraits(memberZ, INDICATOR_IDS)),
        size: members.length,
        medianScore: median(members.map((id) => portfolio.get(id)!.months[index].score)),
  ...
  peerSpaceCache = { fit, coords, months };
  return peerSpaceCache;
}
 ...
 */
export function getPeerMap(input: {
  month?: string;
  scope: Scope;
  company?: string;
}): PeerMapResponse {
  const { month, index } = monthIndexOf(input.month);
  const space = peerSpace();
  const portfolio = buildPortfolio();
  const focus = input.company && portfolio.has(input.company) ? input.company : null;
  ...
  const points: PeerPoint[] = [...portfolio.entries()].map(([id, dataset], i) => {
    const all = space.coords.get(id)!;
    const current = dataset.months[index];
    const first = dataset.months[start];
