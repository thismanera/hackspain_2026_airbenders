...
Entrada: **el score y poco más** (decisión 43). El motor de decisión lee
`score`, sus tres pilares `A/B/C`, `confianza`, `estado`, la tendencia
(`direccion`, `naturaleza`, `tend_score_3m`, `tend_3m`), los **tipos** de
alerta, lo que define al grupo (`D1`, `D5`, `aval_grupo`) y **una** variable en
euros, `tamano = media3m(cobros_op)`. Nada más de la fila del score: ni flujos a
6 meses, ni `capacidad_cuota_adv`, ni `racha_B2`, `racha_deficit`, `C3_dias` o
`C4`, ni los flujos consolidados del grupo. Salida por empresa-mes: `elegible`,
`motivo`, `L` (límite), `menu[]` de opciones (plazo, cantidad máx, TAE),
`accion`. Todo validado 19-09 (decisiones 11, 12, 17-23, calibración 39-42 y
contrato mínimo 43-47).
