...
| [4] Producto | Cartera y ficha de empresa hechas en `main` (PRs #23, #24), con selector de mes y export. **Leen fixtures, no la base de datos.** Faltan pantalla de grupo y backtest. Prototipo de diseño en `dashboard-preview.html` (#26). |
...
app/(panel)/cartera/    pantalla cartera y ficha de empresa ([companyId])
components/grifo/       sidebar, selector de mes, export, tabla/filtros/resumen/
                        gráficas de cartera; en company/: cascada, tendencia,
                        decisión, menú, puertas, grupo, alertas, cobertura
lib/features/portfolio/ datos del panel. source.ts es el único punto que hay
                        que cambiar para leer de Prisma en vez de fixtures.ts
dashboard-preview.html  prototipo HTML estático de diseño (referencia, no producto)
```
...
- Pantallas de cartera y ficha de empresa (Noel, PRs #23 y #24): selector
  de mes, filtros, tabla, resumen y gráficas; en la ficha, cascada,
  tendencia, decisión, menú, puertas, bloque de grupo y alertas. Con datos
  de fixtures.
...
- PRs apiladas sin mergear: #20 scoring → #21 decisión → #22 docs jurado.
  Hasta que entren, `main` tiene el motor de score antiguo y la UI no
  puede leer datos reales.

### 8.3 Falta ✘ (por orden de valor para la demo)

1. **Conectar la UI a datos reales**: sustituir `fixtures.ts` por Prisma en
   `lib/features/portfolio/source.ts` cuando entren #20 y #21. Es lo que
   el jurado ve.
2. **Pantallas de grupo y backtest** (§4.1). Nombre en la UI: hoy pone
   "Grifo"; la decisión 33 dice **Embat Flow**.
3. **Calibrar el backtest de alerta** (§9.3): un recall de 0,14 no vende
   anticipación. Palancas: umbral de dirección ±6, persistencia de 2 meses,
   definición de evento (3 meses de déficit).
4. **Golden path**: elegir las empresas X, Y, Z reales del backtest (§4.3).
5. **Vídeo y demo**: obligatorios además del repo.
6. Alertas: feed en cartera y webhook a Slack (bonus del reto).
7. Entrega test: correr el pipeline con `version_parametros` congelada
   sobre las 60-80 empresas nuevas y exportar el CSV. El export existe;
   falta probarlo con datos nuevos.
8. `PRODUCT.md` está vacío: rellenarlo o hacer que apunte a este documento.
9. Botón "explicar en palabras" con LLM: solo si sobra tiempo.
